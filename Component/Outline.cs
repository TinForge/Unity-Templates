// Outline created by Chris Nolet on 3/30/18.  Copyright © 2018 Chris Nolet. All rights reserved.
using System;
...
namespace VRVision.Quest
{
	...
	{
		[Help("Adds outline effect to object.")]
		...
		//[Header("Optional"), SerializeField, Tooltip("Precompute enabled: Per-vertex calculations are performed in the editor and serialized with the object. "
		...
		void Awake()
		{
			...
			if (outlineMaskMaterial == null || outlineFillMaterial == null)
				Debug.Log("Outline couldn't find Materials, was Resources folder renamed?");
		...
		{
			if (flashEffect)
				OutlineWidth = Mathf.PingPong(Time.realtimeSinceStartup, 1) * flashSize;
